...
class JSONFileHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        # The 'result' of run_coroutine_threadsafe is an asyncio.Future object.
        # It's good practice to add a done callback if you need to handle exceptions
        # or results from the broadcast in the Watchdog thread. For simple fire-and-forget,
        # just scheduling it is enough to get rid of the RuntimeWarning and Runtime Error.
        self.loop.call_soon_threadsafe(
            asyncio.create_task,
            self.broadcast_message(message)
        )
        print(f"[WEBSOCKET] Enviando mensaje a {len(CONNECTED_CLIENTS)} clientes.")
        # print(message) # Uncomment if you want to see the full message in console
        print()

    async def broadcast_message(self, message: str):
        """Envía un mensaje a todos los clientes WebSocket conectados."""
        if CONNECTED_CLIENTS:
            # Use `await asyncio.gather` for concurrent sending to all clients
            # and `return_exceptions=True` to prevent one client error from stopping others.
